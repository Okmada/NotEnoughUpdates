...
//	private int buildupSoundDelay = 0;
	...
	private EntityFishHook hookEntity = null;
	private EntityArmorStand fishWarningEntity = null;
	private final HashMap<Integer, EntityArmorStand> possibleWarningsEntities = new HashMap<>();
	private double lastDistance;
	private long lastCastRodMillis = 0;
	private int etaTicks = 0;
	private int pingDelayTicks = 0;
	private final List<Integer> pingDelayList = new ArrayList<>();
	...
	public void addEntity(int entityId, Entity entity) {
		if (entity instanceof EntityFishHook) {
			if (((EntityFishHook) entity).angler == Minecraft.getMinecraft().thePlayer) {
				hookEntity = (EntityFishHook) entity;
			...
			if ((hookEntity != null && !hookEntity.isDead) && (fishWarningEntity == null || fishWarningEntity.isDead)) {
				possibleWarningsEntities.put(entityId, (EntityArmorStand) entity);
			}
		}
	}

	public void removeEntity(int entityId) {
		possibleWarningsEntities.remove(entityId);
	}

	@SubscribeEvent
	public void onWorldUnload(WorldEvent.Unload event) {
		fishWarningEntity = null;
		hookEntity = null;
		possibleWarningsEntities.clear();
	}
		...
		if (Minecraft.getMinecraft().thePlayer != null && event.phase == TickEvent.Phase.END) {
			...
			if (NotEnoughUpdates.INSTANCE.config.fishing.incomingFishWarning ||
				...
				if (hookEntity != null && !hookEntity.isDead) {
					...
					if (fishWarningEntity == null || fishWarningEntity.isDead) {
						for (EntityArmorStand armorStand : possibleWarningsEntities.values()) {
							if (armorStand.getCustomNameTag().equals("§e§l?") || armorStand.getCustomNameTag().equals("§c§l!!!")) {
								fishWarningEntity = armorStand;
								possibleWarningsEntities.clear();
								lastDistance = -1;
								break;
							}
						...
						double dX = fishWarningEntity.posX - hookEntity.posX;
						double dZ = fishWarningEntity.posZ - hookEntity.posZ;
						double dist = Math.sqrt(dX * dX + dZ * dZ);

						if (lastDistance != dist) {
							etaTicks = (int) Math.floor(dist / 0.1f);
						} else {
							etaTicks--;
						}

						if (etaTicks <= pingDelayTicks) {
							warningState = PlayerWarningState.FISH_HOOKED;
						} else {
							warningState = PlayerWarningState.FISH_INCOMING;
						}
...
//
		if ((particleType == EnumParticleTypes.WATER_WAKE || particleType == EnumParticleTypes.SMOKE_NORMAL || particleType == EnumParticleTypes.FLAME) && Math.abs(
			...
			double fishX = fishWarningEntity.getPosition().getX();
			double fishY = fishWarningEntity.getPosition().getY();
			double fishZ = fishWarningEntity.getPosition().getZ();

			double distX = Math.abs(x - fishX);
			double distY = Math.abs(y - fishY);
			double distZ = Math.abs(z - fishZ);

			double distXZ = Math.sqrt(distX * distX + distZ * distZ);
			System.out.printf( "%f %f %f %f %f %f %f\n", distX, distY, distZ, distXZ, xOffset, yOffset, zOffset);
		}}
